...
**Input**: User description: "Project Management Features — Structured Organization: Organize work into dedicated projects containing resources, milestones, and roadmaps. Granular Access & Visibility: Projects are private, visible strictly to assigned project members. Member Capabilities: Project members can view, edit, and update active project data and resources. Administrative Controls: Only admins can create projects, manage project membership (adding or removing members), and archive existing projects. Archived Project Restrictions: When a project is archived, project members retain read-only access and can no longer edit or update project data. A project has: project dates, name, description, resources, roadmaps, and milestones. Resources are just text and links."
...
1. **Given** a logged-in admin, **When** they create a new project with a name, description, start date, and end date, **Then** the project is created with an active status and is not visible to any non-member user.
...
A project member accesses their assigned project, browses resources, milestones, and roadmaps, and makes updates.
...
**Independent Test**: Can be fully tested by a member opening a project and successfully updating a resource, milestone, or roadmap entry.

**Acceptance Scenarios**:

1. **Given** a user who is a member of an active project, **When** they navigate to the project, **Then** they can view the project's name, description, dates, resources, milestones, and roadmaps.
2. **Given** a member viewing an active project, **When** they add or edit a resource (text and links), **Then** the change is saved and visible to all other project members.
...
3. **Given** an archived project, **When** a member views the project, **Then** all resources, milestones, and roadmaps are fully readable.
...
- What happens if a project's end date is in the past? The project remains active until explicitly archived.
...
- **FR-002**: A project MUST contain: a name, a description, a start date, an end date, resources, milestones, and roadmaps.
- **FR-003**: A resource MUST consist of text content and one or more links.
- **FR-004**: Projects MUST be private; a project's existence and content MUST NOT be visible to users who are not assigned members.
- **FR-005**: Only users with the admin role MUST be able to add members to a project.
- **FR-006**: Only users with the admin role MUST be able to remove members from a project.
- **FR-007**: Project members MUST be able to view all project data (name, description, dates, resources, milestones, roadmaps) within any active project they belong to.
- **FR-008**: Project members MUST be able to create, edit, and delete resources within any active project they belong to.
- **FR-009**: Only users with the admin role MUST be able to archive a project.
- **FR-010**: When a project is archived, all project members MUST retain read access to all project data.
- **FR-011**: When a project is archived, project members MUST NOT be able to create, edit, or delete any project resource, milestone, or roadmap.
- **FR-012**: The system MUST enforce access control at the project level so that unauthenticated users and non-member authenticated users cannot access project data.
- **FR-013**: Only users with the admin role MUST be able to reactivate an archived project.
- **FR-014**: When an archived project is reactivated, all project members MUST immediately regain full read and write access to all project data.

### Key Entities

- **Project**: Represents a workspace for organizing work. Key attributes: name, description, start date, end date, status (active or archived), created by, created at.
...
- **Resource**: A free-form content entry within a project. Key attributes: text content, links (one or more URLs), project reference, created by, last updated by, last updated at.
- **Milestone**: A time-bound goal or checkpoint within a project. Internal structure is out of scope for this feature.
- **Roadmap**: A high-level plan or timeline within a project. Internal structure is out of scope for this feature.

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: An admin can create a new project with all required fields and add at least one member within 2 minutes of starting the workflow.
- **SC-002**: 100% of access attempts to a private project by non-members are denied, with no project data leaked.
- **SC-003**: A project member can view and update any resource, milestone, or roadmap in an active project without admin intervention.
- **SC-004**: Project write access is revoked for all members within 1 second of an admin archiving a project.
- **SC-005**: Members of an archived project can successfully read all project data without interruption.
...
- **SC-008**: A project member can add a resource with text and at least one link in under 1 minute.
...
- "Project dates" means a start date and an end date (target completion date). Both are optional at creation.
- Resources consist of free-form text and one or more links (URLs). No file attachments or structured fields are required in v1.
- Milestones and roadmaps are treated as project sub-items; their internal structure and editing interfaces are out of scope for this feature.
...
- A project with a past end date remains active until explicitly archived by an admin.
